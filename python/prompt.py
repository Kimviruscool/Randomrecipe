...
def build_and_run_prompt(client, data):
    ...
    if mode == "cook":
        ...
        final_prompt = f"나는 지금 '{ingredients}' 재료를 가지고 있어. 이걸로 '{category}' 스타일의 요리를 만들고 싶은데, 랜덤으로 메뉴 1개와 간단한 레시피를 추천해줘. 단 오직 있는재료만 사용해야해 추가적인 재료는 없어. 결과를 줄때 필요재료, 조리방법, 더맛있게만드는 tip을 알려줘 그리고 결과앞에 귀여운 이모지도 하나씩 달아줘. 조리 방법은 1~10 으로 숫자로 조리방법을 표시해줘. 팁은 간단하게 단 한가지의 팁만 줘"
