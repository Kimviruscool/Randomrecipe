...
def build_and_run_prompt(client, data):
    ...
    if mode == "cook":
        ...
        final_prompt = f"나는 지금 '{ingredients}' 재료를 가지고 있어. 이걸로 '{category}' 스타일의 요리를 만들고 싶은데, 랜덤으로 메뉴 1개와 간단한 레시피를 추천해줘."

    # [배달/외식] 모드일 때
    else:
        flavors = ", ".join(data.get("flavors", []))
        final_prompt = f"지금 '{category}' 음식이 당기는데, 특히 '{flavors}' 특징이 있는 메뉴로 딱 하나만 골라서 추천해줘. 추천 이유도 짧게 알려줘."
